...
import { ArrowLeft, Search, Plus, User, Building2, Users, Phone, SearchCheck } from "lucide-react";
...
const UnifiedTransfer = ({ onBack, initialTab = 'mobile' }: UnifiedTransferProps) => {
        ...
        <div className="flex flex-col h-full bg-background animate-in slide-in-from-right duration-300 fixed inset-0 z-50">
            ...
            <div className="flex items-center justify-between p-4 border-b border-border bg-card/50 backdrop-blur-md sticky top-0 z-10">
                <div className="flex items-center">
                    ...
                    <h2 className="font-bold text-foreground text-lg">Send to person</h2>
                </div>
                {activeTab === 'bank' && (
                    <button
            ...
            <div className="flex-1 overflow-y-auto p-4">
                ...
                {activeTab === 'mobile' ? (
                    ...
                    <div className="space-y-4">
                        <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider px-1">Saved Beneficiaries</h3>
                        {filteredBeneficiaries.length > 0 ? (
                            filteredBeneficiaries.map((ben) => (
                                <Card
                                    key={ben.id}
                                    className="bg-card border-border hover:bg-muted/50 transition-colors cursor-pointer active:scale-[0.98]"
